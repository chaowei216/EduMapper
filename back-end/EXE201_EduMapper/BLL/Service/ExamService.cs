    ...
    public class ExamService : IExamService
    {
        ...
        public async Task<ResponseDTO> StartExam(ProgressCreateDTO progress)
        {
            ...
            if (checkExist1 != null)
            {
                ...
                if (exam != null && exam.ExamName.ToLower().Trim().Contains(ExamNameConstant.ReadingTest) && test.IsRequired)
                {
                    throw new BadRequestException(GeneralMessage.BadRequest);
                }

                if (exam != null && exam.ExamName.ToLower().Trim().Contains(ExamNameConstant.ListeningTest) && test.IsRequired)
                {
                    throw new BadRequestException(GeneralMessage.BadRequest);
                }
